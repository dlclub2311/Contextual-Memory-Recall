...
class IncrementalDataset:
    ...
    def get_seen_dataloaders(self):
        print(f"self._current_task {self._current_task} Current high_class = {sum(self.increments[:self._current_task])}")
        max_class = sum(self.increments[:self._current_task])
        x_train, y_train = self._select(self.data_train, self.targets_train, high_range = max_class)
        x_test, y_test = self._select(self.data_test, self.targets_test, high_range = max_class)

        train_loader = self._get_loader(x_train, y_train, np.zeros((x_train.shape[0],)), mode="train")
